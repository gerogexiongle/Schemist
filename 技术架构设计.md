...
**2026-09 更新**：Web、飞书和 MCP 的完整问数统一由 `skills/sql_pipeline.py` 编排；增加 `mcp_api.py`、`skills/complex_query_retrieval.py`、`skills/sql_execution_errors.py`，以及工具预算、模型兜底、Trino 预检、跨入口历史和状态快照。新增模块与配置的完整说明见 [功能与配置升级](./ai_service_2/docs/FEATURE_UPGRADE.md)；下文为基础架构说明。

```mermaid
flowchart LR
    Web[Web] --> Pipeline[统一查询流水线]
    Feishu[飞书] --> Pipeline
    MCP[MCP] --> Pipeline
    Pipeline --> Generate[SQL 生成与模型兜底]
    Generate --> Schema[Schema 检索与漏斗阶段检查]
    Generate --> Check[SQL 预校验]
    Check --> Execute[Spark / Trino 执行]
    Execute --> Repair[错误分类与修复]
    Repair --> Check
    Execute --> Analyze[分析报告]
    Pipeline --> Trace[查询历史与状态快照]
```
...
- **Skills Pack** 的长窗口查询应遵循：字面量分区范围、减少重复扫表、JOIN 键类型与引擎一致、避免不支持的 hint 语法。具体指标和关联规则由部署者在本地技能中定义。
